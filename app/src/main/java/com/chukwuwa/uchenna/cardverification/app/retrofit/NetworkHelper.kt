...
suspend fun <T> safeApiCall(dispatcher: CoroutineDispatcher, apiCall: suspend () -> Response<T>): ResponseHelper<T> {
...
sealed class ResponseHelper<out T> {
    ...
    data class  GenericError(val code: Int? = null, val error: String? = null): ResponseHelper<Nothing>()
    data class NetworkError(val message: String? = "Error during operation please check your network"): ResponseHelper<Nothing>()
}